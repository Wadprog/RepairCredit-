...
router.post("/", async (req, res) => {
  ...
  try {
    if (!level)
      return res.status(400).json({ errors: [{ msg: "Specify user access" }] });
    if (!reference)
      return res
        .status(400)
        .json({ errors: [{ msg: "Specify user reference" }] });

    let user = await User.findOne({ email });
    if (user)
      return res.status(400).json({ errors: [{ msg: "This email is taken" }] });

    user = await User.findOne({ reference });
    if (user)
      return res
        .status(400)
        .json({ errors: [{ msg: "A User axist for this Perseon" }] });
    ...
    jwt.sign(
      ...
      (err, token) => {
        if (err)
          return res
            .status(500)
            .json({ errors: [{ msg: "Error al crear token ${err}" }] });
